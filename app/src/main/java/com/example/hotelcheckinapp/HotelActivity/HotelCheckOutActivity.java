...
public class HotelCheckOutActivity extends AppCompatActivity {
    TextView HotelName;
    String Hname;
    String OCode;
    EditText Cname;
        ...
        Submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(Cname.getText().toString().contentEquals("")){
                    ...
                    sendDetails();
                    Toast.makeText(HotelCheckOutActivity.this, "Succesfully Check Out", Toast.LENGTH_SHORT).show();
                    Intent i = new Intent(HotelCheckOutActivity.this, HotelMailSActivity.class);
                    startActivity(i);
                    finish();
                }
            }
    ...
    public void sendDetails(){
        ...
        databaseReference.child("Hotel").child("CheckOutDetails").child(OCode).child(User).child("Email").setValue(UserEmail);
        databaseReference.child("Hotel").child("CheckOutDetails").child(OCode).child(User).child("Date Of Birth").setValue(UserDob);
        databaseReference.child("Hotel").child("CheckOutDetails").child(OCode).child(User).child("Addtres").setValue(UserAddr);
        databaseReference.child("Hotel").child("CheckOutDetails").child(OCode).child(User).child("Phone No").setValue(UserPh);
        databaseReference.child("Hotel").child("CheckOutDetails").child(OCode).child(User).child("Entry Time").setValue(UserEntryTime);
        databaseReference.child("Hotel").child("CheckOutDetails").child(OCode).child(User).child("Date").setValue(UserDate);
        databaseReference.child("Hotel").child("CheckOutDetails").child(OCode).child(User).child("Addhar CardNo").setValue(UseAddhar);
    }
}